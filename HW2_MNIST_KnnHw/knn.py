...
class KNN:
    def __init__(self, k, x, y, label_names):
        self.k = k  # k 값
        self.features = x
        self.target = y
        self.label_names = label_names
        self.weighted_majority_vote_value = 0  # 가중치 고려된 majority vote 후 근접한 데이터
        self.majority_vote_value = ''  # majority vote 후 근접한 데이터들
        self.nearest_k = []  # K 개의 테스트 데이터와 근접한 데이터의 인덱스
        self.nearest_dist = []  # 테스트 데이터와 K 개의 근접한 데이터의 길이

    # 두 4차원 벡터의 거리를 맨하탄 거리 계산 방식으로 구한다.
    def distance_metric(self, a, b):
        dist = 0
        for i in range(len(self.features[0])):
            dist += int(abs(a[i]-b[i]))
        return dist

    # 가장 가까운 k개의 데이터가 어떤 것인지 찾아내는 함수(k는 3, 5, 10..)s
    ...
    def majority_vote(self):
        ...
        # 가장 많은 class 가 속한 이름을 찾아 저장한다.
        self.majority_vote_value = self.label_names[majority_cnt.index(max(majority_cnt))]
    ...
    def weighted_majority_vote(self):
        ...
        for i in range(self.k):
            ...
            majority_cnt[self.target[self.nearest_k[i]]] += (weight * -1)
            # print(majority_cnt)
        # 가장 높은 수치를 가진 꽃 class 가 속한 이름을 찾아 저장한다.
        self.weighted_majority_vote_value = self.label_names[majority_cnt.index(max(majority_cnt))]
